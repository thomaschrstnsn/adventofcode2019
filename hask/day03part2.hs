...
data LineSegment = 
    Horizontal { hY :: Int, hXs :: (Int, Int)} |
    Vertical   { vX :: Int, vYs :: (Int, Int)} 
    deriving (Show, Eq)
...
range (x1, x2) =
  let xmin = min x1 x2
      xmax = max x1 x2
      r = [xmin .. xmax]
  in if xmin == x1 then r else reverse r
...
trace :: [[Point]] -> [LineSegment] -> [[Point]]
trace r [] = reverse r
trace r (l:ls) = trace (trace' l : r) ls
  where
    trace' :: LineSegment -> [Point]
    trace' Horizontal {hY, hXs} = drop 1 [(x, hY) | x <- range hXs]
    trace' Vertical   {vX, vYs} = drop 1 [(vX, y) | y <- range vYs]

stepsFromLines :: [LineSegment] -> [Step]
stepsFromLines lss = zip (concat $ trace [] lss) (iterate (+ 1) 1)

pointFromSteps :: [Step] -> Point -> Int
pointFromSteps steps p = snd . head $ filter ((== p) . fst) steps
...
solve sp = minimum (ordering <$> candidates)
 where
  psp = mapBoth readInput sp
  lsp = mapBoth linesFromPaths psp
  candidates = intersections lsp
  ssp = mapBoth stepsFromLines lsp
  ordering p = 
    let
      (fx, fy) = mapBoth pointFromSteps ssp
    in 
      fx p + fy p
